...
# #Get project numbers for TSBT
gcloud asset search-all-resources   --scope='folders/536710404454'  \
    --asset-types='cloudresourcemanager.googleapis.com/Project' \
    --format="value(project)" |  cut -d'/' -f2 > project.txt

# #Get project numbers for GIDC. Append to existing list
gcloud asset search-all-resources   --scope='folders/960225691252'  \
    --asset-types='cloudresourcemanager.googleapis.com/Project' \
    --format="value(project)" |  cut -d'/' -f2 >> project.txt
...
AUTH_CODE=$(curl --request POST \
  ...
  --data '{"lifetime":"600s","scope":["https://www.googleapis.com/auth/admin.directory.user","https://www.googleapis.com/auth/admin.directory.domain.readonly"]}' \
